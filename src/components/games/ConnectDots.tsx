...
const SHAPES = [
  ...
  },
  {
    ...
    emoji: "🏆",
    dots: [
      { x: 50, y: 8 }, { x: 75, y: 25 }, { x: 68, y: 60 },
      { x: 50, y: 80 }, { x: 32, y: 60 }, { x: 25, y: 25 },
    ],
  },
  {
    name: "Estrella del Campo",
    emoji: "⭐",
    dots: [
  ...
  },
  {
    ...
    emoji: "🏟️",
    dots: [
      { x: 10, y: 75 }, { x: 10, y: 40 }, { x: 30, y: 20 },
      { x: 50, y: 15 }, { x: 70, y: 20 }, { x: 90, y: 40 }, { x: 90, y: 75 },
    ],
  },
];
...
export default function ConnectDots() {
  ...
  const reset = useCallback(() => {
    setShapeIdx(0); setNextDot(0);
    setConnected([]); setScore(0);
    setGameOver(false); setStarted(false); setFlash(null);
  }, []);

  const tapDot = (idx: number) => {
    if (!started || gameOver || idx !== nextDot) return;
    setFlash(idx);
    ...
    if (newNext >= shape.dots.length) {
      ...
      setTimeout(() => {
        ...
        if (nextShape >= SHAPES.length) setGameOver(true);
        else { setShapeIdx(nextShape); setNextDot(0); setConnected([]); }
      }, 800);
    } else {
      setNextDot(newNext);
    }
    ...
    >
      <div
        ...
        style={{
          ...
          background: "linear-gradient(135deg, #0a1a3d 0%, #0d3d0d 100%)",
        }}
      >
        {!started && !gameOver && (
          <div className="absolute inset-0 flex flex-col items-center justify-center z-10 px-6">
            <div className="text-8xl mb-4 float-anim">📋</div>
            <h2 className="text-3xl font-black text-white text-center mb-3">Dibuja la Jugada</h2>
            <p className="text-white/70 text-center mb-8 text-sm leading-relaxed">
              El entrenador te da el esquema. ¡Conecta los puntos en orden para dibujar la jugada!
            </p>
            <button
          ...
          <>
            <div className="text-center pt-3 pb-1">
              <span className="text-white/60 text-sm">
                Figura {shapeIdx + 1}/{SHAPES.length}: {shape.name} {shape.emoji}
              </span>
            </div>
            ...
            <div className="relative w-full" style={{ height: "55vw", maxHeight: 340 }}>
              <svg className="absolute inset-0 w-full h-full" viewBox="0 0 100 100" preserveAspectRatio="xMidYMid meet">
                ...
                {connected.slice(0, -1).map((_, i) => (
                  <line
                    key={i}
                    x1={shape.dots[i].x} y1={shape.dots[i].y}
                    x2={shape.dots[i + 1].x} y2={shape.dots[i + 1].y}
                    stroke="#3498DB" strokeWidth="2" strokeLinecap="round"
                  />
                ))}
                ...
                {shape.dots.map((dot, idx) => {
                  ...
                  return (
                    <g key={idx} onClick={() => tapDot(idx)} style={{ cursor: "pointer" }}>
                      <circle
                        cx={dot.x} cy={dot.y} r={isNext ? 5 : 4}
                        fill={isDone ? "#3498DB" : isNext ? "#FFD700" : "rgba(255,255,255,0.3)"}
                        stroke={isNext ? "#FFD700" : isDone ? "#3498DB" : "rgba(255,255,255,0.5)"}
                        strokeWidth={flash === idx ? 2 : 1}
                      />
                      {!isDone && (
                        <text x={dot.x} y={dot.y + 0.5} textAnchor="middle" dominantBaseline="middle" fontSize="3.5" fill={isNext ? "#000" : "rgba(255,255,255,0.7)"} fontWeight="bold">
                          {idx + 1}
                        </text>
                      )}
                      {isDone && (
                        <text x={dot.x} y={dot.y + 0.5} textAnchor="middle" dominantBaseline="middle" fontSize="4" fill="white">✓</text>
                      )}
                    </g>
                  );
                })}
              ...
              {shape.dots.map((_, idx) => {
                ...
                return (
                  <button
                    ...
                    style={{
                      background: isDone ? "rgba(52,152,219,0.4)" : isNext ? "rgba(255,215,0,0.3)" : "rgba(255,255,255,0.05)",
                      border: isDone ? "2px solid #3498DB" : isNext ? "2px solid #FFD700" : "1px solid rgba(255,255,255,0.1)",
